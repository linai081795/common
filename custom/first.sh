...
Diy_three() {
    ...
    if [[ "$SYNCHRONISE" == "NO" ]]; then
        if [[ "${BENDI_VERSION}" == "1" ]]; then
            shangyou=$(mktemp -d)
            if git clone --single-branch --depth=1 --branch=main https://github.com/linai081795/build-actions "${shangyou}"; then
            ...
            if ! git clone -q --single-branch --depth=1 --branch=main https://github.com/linai081795/build-actions shangyou; then
              git clone --depth=1 https://github.com/linai081795/build-actions shangyou
            fi
            if [[ ! -d "repogx" ]]; then
